...
void updateTurningControl(float target_rad) {
  // 1. Calculate the shortest path to the target
  float heading_error = getSmallestAngleDiff(target_rad, pose.theta);
  display.gotoXY(0,0);
  display.print(pose.theta);
  display.gotoXY(0,1);
  display.print(heading_error);
  Serial.println(heading_error);
  // 2. Scale error to a demand (Proportional Control)
  float turn_demand = heading_error * turn_gain;

  // 3. Safety: Constrain the demand so the PID can handle it
  turn_demand = constrain(turn_demand, -max_turn_speed, max_turn_speed);

  // 4. Set motor demands (Left forward, Right backward for positive error)
  left_demand = -turn_demand;
  right_demand = turn_demand;

  // 5. Optional: Deadband
  // If the error is tiny, just stop to prevent "hunting" (vibrating)
  if (abs(heading_error) < 0.01) { // approx 1 degree
    left_demand = 0;
    right_demand = 0;
    left_pid.reset();
    right_pid.reset();
...
void loop(){
  ...
  if (millis() - test_ts > 4000) {
    test_target = (test_target == PI) ? 0.0 : PI ;
    test_ts = millis();
  }
  
  updateTurningControl(test_target);
