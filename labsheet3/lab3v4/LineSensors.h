...
class LineSensors_c {
  ...
  public:
    ...
    void calibrate() {
      readSensorsADC();
      // 2. Loop through each sensor and update min/max
      for( int i = 0; i < NUM_SENSORS; i++ ) {
        // If the new reading is smaller than our stored minimum, update it
        if( readings[i] < minimum[i] ) {
          minimum[i] = readings[i];
        }
        // If the new reading is larger than our stored maximum, update it
        if( readings[i] > maximum[i] ) {
          maximum[i] = readings[i];
        }
      }
    }
    // This function sets the initial values for the calibration process
    void initialiseCalibration() {
      for( int i = 0; i < NUM_SENSORS; i++ ) {
        minimum[i] = 1023.0; // Start high so any reading will be lower
        maximum[i] = 0.0;    // Start low so any reading will be higher
      }
    }
    void setScaling(){
      for( int i = 0; i < NUM_SENSORS; i++){
        scaling[i] = maximum[i] - minimum [i];
      }
    }
    ...
    void calcCalibratedADC() { 
      readSensorsADC();  
      for( int i = 0; i < NUM_SENSORS; i++ ) {
        calibrated[i] = (readings[i] - minimum[i]) / scaling[i];
      }
    } 
    ...
    bool isOnLine(){
      ...
      calcCalibratedADC();
      for(int i = 0; i < NUM_SENSORS; i++){
        if(calibrated[i] > 0.5) onLine = true;
      }
      return onLine;
    }
